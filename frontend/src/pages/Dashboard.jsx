...
const Dashboard = () => {
    ...
    const stats = [
        ...
        },
        {
            ...
            sub: `የ ${new Date().getFullYear() - 8 + ((new Date().getMonth() > 8 || (new Date().getMonth() === 8 && new Date().getDate() >= 11)) ? 1 : 0) - 1}/${new Date().getFullYear() - 8 + ((new Date().getMonth() > 8 || (new Date().getMonth() === 8 && new Date().getDate() >= 11)) ? 1 : 0)} ተመራቂ`,
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-700">
            <SEO title="ዳሽቦርድ" />
            {isManager && (
                <div className="bg-blue-900/5 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-900/10 dark:border-blue-900/30 mb-6">
                    <h2 className="text-lg font-bold text-blue-900 dark:text-blue-100">የስራ አስኪያጅ አጠቃላይ እይታ</h2>
                    <p className="text-sm text-gray-600 dark:text-blue-200/70">በሁሉም ክፍሎች ያሉ አጠቃላይ ስታቲስቲክስን እየተመለከቱ ነው።</p>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <motion.div
                        ...
                        key={stat.label}
                        className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-premium dark:shadow-none border border-gray-100 dark:border-slate-700 group hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-default"
                    >
