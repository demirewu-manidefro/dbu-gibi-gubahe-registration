    ...
    const recordActivity = async (type, details) => {
        ...
        try {
            ...
            if (type !== 'registration' && type !== 'student_update' && type !== 'message' && type !== 'login') {
                ...
                if (message) {
                    // We don't locally set notifications here anymore as they are generated by backend
                    // and fetched via polling/refresh.
                    // If we want immediate feedback for *self* actions, we can fetchNotifications()
                    fetchNotifications();
                }
            }
        } catch (err) {
